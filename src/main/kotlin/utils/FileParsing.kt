...
fun execFileByLine(number: Int, f: (str: String) -> Unit) =
    File("./src/main/kotlin/inputs/input$number.txt").forEachLine { f(it) }

fun execFileByLineIndexed(number: Int, f: (index: Int, str: String) -> Unit) {
    val arr = mutableListOf<String>()
    File("./src/main/kotlin/inputs/input$number.txt").forEachLine {
        arr.add(it)
    }
...
fun numberOfLinesPerFile(number: Int): Int {
    val reader = BufferedReader(FileReader("./src/main/kotlin/inputs/input$number.txt"))
    var lines = 0
